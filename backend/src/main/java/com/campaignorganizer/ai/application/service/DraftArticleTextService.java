...
/**
 * Drafts article text: builds the level/kind-specific prompt, then delegates
 * provider selection and fallback to {@link ProviderFallbackTextGenerator}
 * (ADR-0065). No transaction boundary of its own; this call writes nothing.
 */
@Service
public class DraftArticleTextService implements DraftArticleTextUseCase {
    ...
    public DraftArticleTextService(ProviderFallbackTextGenerator generator) {
    ...
    public DraftResult draft(DraftArticleTextCommand command) {
        ...
        return generator.generate(systemPrompt, userPrompt);
    }
