...
/** Thin web adapter for a statblock's tags (ADR-0083). */
@RestController
@RequestMapping("/api/worlds/{worldId}/statblocks/{statblockId}/tags")
public class StatblockTagController {
    ...
    public StatblockTagController(ListEntityTagsUseCase listUseCase,
                                  SetEntityTagsUseCase setUseCase, TagWebMapper mapper) {
    ...
    @GetMapping
    public EntityTagsResponse get(@PathVariable UUID worldId, @PathVariable UUID statblockId) {
        return mapper.toResponse(listUseCase.list(worldId, EntityType.STATBLOCK, statblockId));
    }

    @PutMapping
    public EntityTagsResponse set(@PathVariable UUID worldId, @PathVariable UUID statblockId,
                                  @Valid @RequestBody EntityTagsRequest request) {
        return mapper.toResponse(setUseCase.set(
                mapper.toSetCommand(worldId, EntityType.STATBLOCK, statblockId, request)));
    }
}
