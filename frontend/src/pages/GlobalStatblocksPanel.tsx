...
/**
 * World-independent, system-scoped statblock catalog (ADR-0096) — real
 * monster/NPC instances, reused across worlds by explicit copy-on-import
 * into a campaign (that action lives on the world-scoped StatblocksPanel,
 * which has world/campaign context this page doesn't).
 */
export function GlobalStatblocksPanel({ onAuthExpired }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      edit(saved);
      navigate(`/templates/statblocks/${saved.id}`);
      refresh();
          ...
          {list.map((s) => (
            <li key={s.id} className="rel-row">
              <Button
                ...
                onClick={() => navigate(`/templates/statblocks/${s.id}`)}
              >
