...
/**
 * /next's own fork of TimelinesView (ADR-0106) — a straight duplicate, no
 * markup changes needed: already uses /next's shared .wiki-layout/
 * .wiki-sidebar/.wiki-main classes and path-relative navigation.
 */
export function NextTimelinesView({ worldId, onOpenArticle, onAuthExpired }: Props) {
  ...
  async function createTimeline(name: string) {
    try {
    ...
    <div className="wiki-layout" data-has-selection={!!selected}>
      <aside className="wiki-sidebar">
        ...
        <PromptDialog
          ...
          title="New timeline"
          label="Timeline name"
          onSubmit={(name) => void createTimeline(name)}
        />
